...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val registerButtonStream = Observable.combineLatest(
            emailStream,
            passwordStream,
            confirmPasswordStream
        ) { emailInvalid: Boolean, passwordInvalid: Boolean, confirmPasswordInvalid: Boolean ->
            !emailInvalid && !passwordInvalid && !confirmPasswordInvalid
        }
        registerButtonStream.subscribe { isValid ->
            if (isValid) {
